...
function BootStrapScript(){
	...
	if (typeof $().modal != 'function' || typeof jQuery().modal != 'function'){
	...
	jQuery.ajax({
		...
		// jQuery(window).scrollTop(0);
			...
			jQuery('#ms_modal_remove_button').click(function(){
				...
				msCookies.set('mswishlist',mswishlist_json, {sameSite: 'None', secure: true} ,{ expires: 365 });
		...
		if (confirm(jQuery('#msg_clear_all').val())) {
					...
					msCookies.set('mswishlist',JSON.stringify({"items":[]}),{sameSite: 'None', secure: true},{ expires: 365 });
					wishlist_save();
					setTimeout(function(){ 
					location.reload();
					}, 500);
