        ...
        if(isset($_GET['charge_id'])){
            ...
            $token = $this->session->userdata('token');
             ...
             try {
                ...
                $charges = $this->shopifyapi->call('GET', '/admin/api/2020-01/recurring_application_charges/'.$charge_id.'.json');
                
                if($charges['status'] == 'declined'){
                    
                    $this->charges->remove_data(getstore('id'));
                    //redirect('https://'.$shop.'/admin/apps');
                    ?>
                <script>
                window.top.location = "https://<?php echo $shop; ?>/admin/apps";
                </script>
                <?php
                exit;
                    ...
                    $activate = $this->shopifyapi->call('POST', '/admin/api/2020-01/recurring_application_charges/'.$charge_id.'/activate.json',$data);
